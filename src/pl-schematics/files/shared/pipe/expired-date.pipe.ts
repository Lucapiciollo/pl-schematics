   ...
   transform(value: string): string {
      if (value && moment(value).isValid()) {
         if (moment().diff(moment(value), 'minutes') > 30) {
            if (this.element.nativeElement.parentElement) this.element.nativeElement.parentElement.classList.toggle('expired');
            else this.element.nativeElement.classList.toggle('expired');
         }
      }
      return value;
   }
