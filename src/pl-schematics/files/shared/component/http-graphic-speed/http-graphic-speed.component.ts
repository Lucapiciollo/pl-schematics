...
@Component({
  selector: '<%=prefix%>-http-graphic-speed', 
  templateUrl: './http-graphic-speed.component.html',
  styleUrls: ['./http-graphic-speed.component.css']
})
@PLUnsubscribe()
export class <%=classify(prefixClass)%>HttpGraphicSpeedComponent implements OnInit, AfterContentInit {
  ...
  @Input() set idAjax(id: string) {
    try {
      ...
      throw new <%=classify(prefixClass)%>ErrorBean(e.message, <%=classify(prefixClass)%>ErrorCode.SYSTEMERRORCODE, false, false);
    }
  };
  /******************************************************************************************* */
 ...
 */
  ...
  constructor(private httpService: <%=classify(prefixClass)%>HttpService) { }

  /********************************************************************************************************************** */
  ngAfterContentInit(): void {
    try {
      this.idChart = <%=classify(prefixClass)%>Utils.UUIDCODE();
  ...
  /********************************************************************************************************************** */
  ngOnInit() { this.idScroll = <%=classify(prefixClass)%>Utils.UUIDCODE(); }
  /********************************************************************************************************************** */
